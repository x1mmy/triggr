...
- **Onboarding Telegram** (required for `/api/submit`): `ONBOARDING_TELEGRAM_BOT_TOKEN` and `ONBOARDING_TELEGRAM_CHAT_ID` — separate from contact-form `TELEGRAM_BOT_TOKEN` / `TELEGRAM_CHAT_ID`. Success alerts include Notion + Linear links only (file URLs stay in Notion Notes).
